        ...
        if (!document.querySelectorAll("th[class *= 'coef ng-star-inserted']")[1]) {
            ...
            for (let i = 0; i < lignes.length; i++) {
                ...
                if (coeffNoteTot ==  0.0) {
					...
					if (!lignes[i].querySelector("[class *= 'kmlc-moyenne']")) {
						lignes[i].querySelector("td.relevemoyenne").innerHTML = lignes[i].querySelector("td.relevemoyenne").innerHTML + '<br><span class="kmlc-moyenne">' + averageElement.innerHTML + '</span>';
					} else {
						lignes[i].querySelector("[class *= 'kmlc-moyenne']").textContent = moyenne.toFixed(5)
					}
            ...
            if (document.querySelector("tr > td.moyennegenerale-valeur")) {
				...
				if (!document.querySelector("[class *= 'kmlc-moyenne-g']")) {
					document.querySelector("tr > td.moyennegenerale-valeur").innerHTML = document.querySelector("tr > td.moyennegenerale-valeur").innerHTML + '<br><span class="kmlc-moyenne-g">' + overallAverageElement.innerHTML + '</span>';
				} else {
					document.querySelector("[class *= 'kmlc-moyenne-g']").textContent = moyenneG.toFixed(5)
				}
			} else {
                let overallAverageElement = document.createElement("tr")
                overallAverageElement.innerHTML = '<tr class="ng-star-inserted"><td colspan="2" class="text-right moyennegeneralelibelle">Moyenne générale</td><td colspan="2" class="moyennegenerale-valeur"><span class="kmlc-moyenne-g">' + moyenneG.toFixed(5); + '</span></td></tr>'
...
function ajouterNote(matiere, titre, note, coeff, quotient) {
	...
	for (let i = 0; i < listMatiere.length; i++) {
		if (listMatiere[i].textContent == matiere) {
			...
			listMatiere[i].parentElement.parentElement.querySelector("[class = 'kmlc-note-simu']").outerHTML = '<button type="button" class="note cliquable margin-whitespace btn-simple no-background no-padding ng-star-inserted" title=" '+ titre + '"><!----><span class="valeur ng-star-inserted" style="color: green;"> ' + note + ' <!----><sup class="coef ng-star-inserted"> ' + coeff + ' <span class="margin-whitespace"></span></sup><!----><sub class="coef ng-star-inserted"> /' + quotient + ' <span class="margin-whitespace"></span></sub></span><!----><!----><!----></button>'
		}
	}
	...
	for (let i = 0; i < lignes.length; i++) {
		...
		if (coeffNoteTot ==  0.0) {
			...
			let averageElement = lignes[i].querySelector("td.relevemoyenne").cloneNode(true);
			averageElement.textContent = moyenne.toFixed(5)
			if (!lignes[i].querySelector("[class *= 'kmlc-simu-moyenne']")) {
				lignes[i].querySelector("td.relevemoyenne").innerHTML = lignes[i].querySelector("td.relevemoyenne").innerHTML + '<br><span class="kmlc-simu-moyenne" style="color: green;">' + averageElement.innerHTML + '</span>';
			} else {
				lignes[i].querySelector("[class *= 'kmlc-simu-moyenne']").textContent = moyenne.toFixed(5)
			}
	...
	if (document.querySelector("tr > td.moyennegenerale-valeur")) {
		...
		if (!document.querySelector("[class *= 'kmlc-simu-moyenne-g']")) {
			document.querySelector("tr > td.moyennegenerale-valeur").innerHTML = document.querySelector("tr > td.moyennegenerale-valeur").innerHTML + '<br><span class="kmlc-simu-moyenne-g" style="color: green;">' + overallAverageElement.innerHTML + '</span>'
		} else {
			document.querySelector("[class *= 'kmlc-simu-moyenne-g']").textContent = moyenneG.toFixed(5)
		}
	} else {
		let overallAverageElement = document.createElement("tr")
		overallAverageElement.innerHTML = '<tr class="ng-star-inserted"><td colspan="2" class="text-right moyennegeneralelibelle">Moyenne générale</td><td colspan="2" class="moyennegenerale-valeur"><span class="kmlc-simu-moyenne-g" style="color: green;">' + moyenneG.toFixed(5); + '</span></td></tr>'
		console.log(10, overallAverageElement, document.querySelector("table.ed-table tbody"))
