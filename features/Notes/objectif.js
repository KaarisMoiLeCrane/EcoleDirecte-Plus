globalThis.Notes.objectifSetup = function () {
	const ajouterObjectifNote = imports("ajouterObjectifNote").from("./features/Notes/functions/ajouter-objectif-note.js")
...
`
			...
			for (let i = 0; i < inputBox.length; i++) {
				...
				// if (inputBoxValue != '') {
					...
					// if (gradeValue != "" && gradeValue != null) {
						...
						// calculerMoyennes(true, "kmlc-simu-moyenne-g", "color: green;", "kmlc-simu-moyenne", "color: green;", true)
	...
	async function applyMeanGoal(subjectGrade, gradeValue, idGrade, save) {
		ajouterObjectifNote(subjectGrade, gradeValue, idGrade)
		// console.log(1111)
		if (!save) return
