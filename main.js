...
new MutationObserver(() => {
	...
	if (url !== lastUrl) {
		...
		if (document.querySelectorAll('div.menu.ed-menu'))
			for (
				let i = 0;
				i < document.querySelectorAll('div.menu.ed-menu').length;
				i++
			)
				document
					.querySelectorAll('div.menu.ed-menu')
					[i].classList.remove('ed-menu');
	}
}).observe(document, {
	subtree: true,
	childList: true,
	...
	if (!lastUrl.includes('/login')) {
		...
		if (globalThis.token && window.location.href.includes('EmploiDuTemps')) {
			if (globalThis.userId != undefined) {
				document
					...
					.then(elm => {
						document
							.getElementById('main-part')
							.classList.remove('sidebarhover');
						// console.log("EDT ", num)
		...
		if (globalThis.token && window.location.href.includes('Messagerie')) {
			if (globalThis.userId != undefined) {
				document
					.waitForElement(
						'[class *= btn-group] > button:not([class *= dropdown])',
					)
					.then(elm => {
						if (document.querySelector('.sidebar:hover'))
							document
								.getElementById('main-part')
								.classList.add('sidebarhover');
						// console.log("MESSAGERIE ", num)
						globalThis.messagerie();
					});
			}
		}
	}
...
function getAccountType() {
	if (window.sessionStorage.accounts)
		return JSON.parse(window.sessionStorage.accounts).payload.accounts[0]
			.typeCompte == "'E'"
			? 'eleves'
			: 'familles';
